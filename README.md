...
* **FW_PREFIX**: firmware installation path in case of firmware-install target.<br>
default: /lib/firmware

Targets:
* *firmware*: compiles the firmware binary file
* *firmware-install*: *firmware* target plus installs the firmware binary under the given **FW_PREFIX**
...
Path:
>/sys/class/tty/ttySI`X`/device/current_rssi
...
Path:
>/sys/class/tty/ttySI`X`/device/package_size
...
Path:
>/sys/class/tty/ttySI`X`/device/rx_channel
...
Path:
>/sys/class/tty/ttySI`X`/device/tx_channel
...
Path:
>/sys/class/tty/ttySI`X`/device/tx_timeout
...
* *driver-install*: *driver* target plus copies the `si4455.ko` under `/lib/modules/$(uname -r)/extra`, then invokes depmod.<br>
  Requires: **root privilages**
* *firmware*: compiles the firmware binary file
* *firmware-install*: *firmware* target plus installs the firmware binary under `/lib/firmware`.<br>
  Requires: **root privilages**
...
* *overlay-install*: *overlay* target plus installs the overlay under `/boot/overlays`.<br>
  Requires: **root privilages**
* *boot-configure*: enables the spi port and adds the overlay in `/boot/config.txt`.<br>
  Requires: **root privilages**
* *install*: *driver-install*, *firmware-install* and *overlay-install* targets.<br>
  Requires: **root privilages**
* *all*: *install* and *boot-configure* targets.<br>
  Requires: **root privilages**
...
![ls -la /dev | grep ttySI](img/rpi_si4455_dev.png)
...
>sudo tio -f none /dev/ttySI0<br>
![sudo tio -f none /dev/ttySI0](img/rpi_si4455_tio.png)

>sudo minicom -c on -D /dev/ttySI1<br>
![sudo minicom -c on -D /dev/ttySI1](img/rpi_si4455_minicom.png)
